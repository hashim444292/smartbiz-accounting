...
export default function PayablesSummaryPage() {
  ...
  const fetchReport = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/reports/payables-summary");
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.setAttribute("download", `Accounts_Payables_Summary.csv`);
  ...
  if (loading && !report) {
    return (
      <div className="space-y-6 max-w-4xl mx-auto pb-12">
        <BrandPageLoader
          message="Loading Payables Summary..."
          submessage="Auditing supplier purchase balances, vendor ledger dues, and payment obligations..."
        />
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">Supplier outstanding balances, debits, credits, and vendor contacts</p>
          </div>
        </div>
      ...
      {loading || !report ? (
        <div className="rounded-xl border border-slate-200 bg-white p-12 dark:border-slate-800 dark:bg-slate-900 shadow-sm">
          <BrandPageLoader
            message="Compiling Accounts Payables..."
            submessage="Reconciling supplier balances, vendor debits, and outstanding purchase invoices..."
          />
        </div>
      ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-t border-b border-slate-400 text-[11px] font-bold text-slate-900">
                  ...
                  <th className="py-1.5 px-3 uppercase tracking-wider font-semibold">SUPPLIER / A/C NAME</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredItems.map((item: any) => (
                  ...
                  >
                    ...
                    <td className="py-1 px-3 text-right tabular-nums text-rose-700">
                ...
                <tr className="border-t-2 border-b-2 border-slate-900 font-bold text-xs text-slate-900 bg-slate-50 print:bg-transparent">
                  ...
                  <td className="py-2 px-3 text-right tabular-nums font-bold text-rose-700">
                    {formatAmt(report.totals.totalCredit)}
                  </td>
                  <td className="py-2 pl-3 text-right font-semibold text-rose-700">
                    Net: {formatMoney(report.totals.netPayable)}
                  </td>
                </tr>
              </tbody>
            </table>
