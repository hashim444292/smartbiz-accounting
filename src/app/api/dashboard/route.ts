...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      [sales, purchases, expenses, products, customers, suppliers] = await Promise.race([
        Promise.all([
          prisma.sale.findMany({ where: { businessId }, include: { items: true }, orderBy: { date: "desc" }, take: 10 }),
    ...
    // Generate monthly revenue trend tailored to this business
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        recentInvoices: fbrOverview.recentInvoices,
        monthlyTrends,
